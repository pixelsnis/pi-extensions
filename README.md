...
- **Plan Mode** starts in Build mode. `/plan` switches to a read-only agent-tool allowlist for planning, then the user reviews the generated plan in a TUI before choosing whether and where to execute it. The allowlist is not an operating-system sandbox; see the [Plan Mode security notes](extensions/plan-mode/README.md#modes-and-tool-gate).
