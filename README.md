...
- **Plan Mode** starts in Build mode. `/plan` switches to a deny-by-default agent-tool gate for planning, with Bash conditionally available only for validated read-only inspection commands and pipelines; the user reviews the generated plan in a TUI before choosing whether and where to execute it. The gate is not an operating-system sandbox; see the [Plan Mode security notes](extensions/plan-mode/README.md#modes-and-tool-gate).
