...
- **Plan Mode** starts in Build mode. `/plan` switches to a deny-by-default agent-tool gate for planning, with Bash conditionally available only for validated read-only inspection commands, pipelines, and `&&` chains. For example, `pwd && ls -la` and `rg -n 'PLAN_TOOLS\b' extensions/plan-mode/index.ts` are accepted; all commands in a pipeline or chain must be allowed. Other chaining/operators remain blocked. The user reviews the generated plan in a TUI before choosing whether and where to execute it. The gate is not an operating-system sandbox; see the [Plan Mode security notes](extensions/plan-mode/README.md#modes-and-tool-gate).
...
Pi extensions run with the process's normal privileges and can execute arbitrary code. Review the source before installing. Plan Mode restricts agent tool calls while planning; Bash is accepted only for simple validated read-only commands, pipelines, and `&&` chains (for example, `pwd && ls -la` or `rg -n '.*;$' extensions/plan-mode/index.ts`). Every command must independently pass the allowlist and option checks. Other shell chaining/operators such as `;`, `||`, and `&`, along with redirection, substitutions, unrecognized commands, mutating/execution options, and interactive `!`/`!!` commands remain blocked. This is not a filesystem, process, or network sandbox; user-invoked extension commands and Pi's own session persistence remain outside that agent-tool restriction. Codex Fast Mode changes outgoing request payloads for its aliases; provider eligibility, charges, and rate limits remain the user's responsibility.
